...
//  FLunarTime.m
//
//  Created by fahlnbg on 22/03/2023.
//

// https://github.com/ios4vn/Vietnamese-Lunar-Calendar---Obj-C

#import "FLunarTime.h"

@implementation FLunarTime
...
+ (int)DayDLnow{
    ...
    NSString *da = [D stringFromDate:date];
    return [da intValue];
}
+ (int)MonthDLnow{
    NSDateFormatter *D = [[NSDateFormatter alloc] init];
...
+ (NSString *)NgayALnow{
    NSMutableArray *lunar = [self Solar2Lunar:[self DayDLnow] :[self MonthDLnow] :[self YearDLnow]];
    return [NSString stringWithFormat:@"%d",[[lunar objectAtIndex:0] intValue]];
}

+ (NSString *)ThangALnow{
    NSMutableArray *lunar = [self Solar2Lunar:[self DayDLnow] :[self MonthDLnow] :[self YearDLnow]];
    if([[lunar objectAtIndex:1] intValue] == 0) {
        return @"12";
    } else
        return [NSString stringWithFormat:@"%d",[[lunar objectAtIndex:1] intValue]];
}

+(NSString *)NamALnow{
    NSMutableArray *lunar = [self Solar2Lunar:[self DayDLnow] :[self MonthDLnow] :[self YearDLnow]];
    if([[lunar objectAtIndex:1] intValue] == 0){
        return [NSString stringWithFormat:@"%d",([self YearDLnow] - 1)];
    } else
        return [NSString stringWithFormat:@"%d",[[lunar objectAtIndex:2] intValue]];
}
...
+(NSString*)NgayAL:(int)day :(int)month :(int)year{
    NSMutableArray *lunar = [self Solar2Lunar:day :month :year];
    return [NSString stringWithFormat:@"%d",[[lunar objectAtIndex:0] intValue]];
}
+(NSString*)ThangAL:(int)day :(int)month :(int)year{
    NSMutableArray *lunar = [self Solar2Lunar:day :month :year];
    if([[lunar objectAtIndex:1] intValue] == 0) {
        return @"12";
    } else
...
+(NSString*)NamAL:(int)day :(int)month :(int)year{
    NSMutableArray *lunar = [self Solar2Lunar:day :month :year];
    if([[lunar objectAtIndex:1] intValue] == 0){
        return [NSString stringWithFormat:@"%d",(year - 1)];
    } else
        return [NSString stringWithFormat:@"%d",[[lunar objectAtIndex:2] intValue]];
}
